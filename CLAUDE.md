...
- **Usage domestique (cuve mixte, juin 2026)** : case « Ajouter l'usage domestique » (`#cDomOn`) qui ajoute au besoin jardin une **demande domestique constante toute l'année** (WC + lave-linge), saisie par **nombre d'occupants** × moyennes L/pers/jour éditables (`DOMESTIQUE` : WC 25, lave-linge 17). Fonction pure `demandeDomestique(occupants, lpjTotal)` → `besoin` mensuel (m³), additionné dans `scen()` via `besoin.push(bm + dom[m])`. ⚠️ `dom[m]` **ne dépend PAS de la pluie** → identique année normale/sèche (les gens tirent autant d'eau quelle que soit la météo) ; la sécheresse ne joue que sur l'**apport** toiture et le besoin **jardin**. Le moteur (`simulCuve`/`dimensionne`/`renderFillChart`) est inchangé (agnostique au type de besoin). Cas **100 % domestique** = supprimer toutes les cultures (le ✕ peut descendre à 0 ligne depuis juin 2026 ; label « par culture — optionnel »), ou surfaces à 0 (`lireZones` filtre `surf>0`). ⚠️ Même travers que le jardin en été sec (Ajaccio 100 m² toit + 50 m² gazon chaud + 4 pers WC/lave-linge → ~20 m³ pour 99 %) **mais** un **appoint réseau** prend toujours le relais (le WC ne tombe jamais à sec) → lire la **couverture %**, pas l'autonomie 100 % ; note `#cuveNoteDom`.
- **Cadre légal domestique** (encart `.dom-legal`, à jour) : l'**arrêté du 21 août 2008 est abrogé** ; depuis le **1ᵉʳ sept. 2024** = **décret 2024-796 + arrêté du 12 juillet 2024**. Autorisés intérieur : WC (sans déclaration), nettoyage des sols, **lave-linge** (nouveauté 2024) sous **déclaration en préfecture + qualité A+** (mention `#cDomLegalLl` affichée quand lave-linge coché). Interdits : boisson, cuisine, vaisselle, hygiène (douche, lavabo). Réseau séparé obligatoire, sans interconnexion, robinets « eau non potable ».
